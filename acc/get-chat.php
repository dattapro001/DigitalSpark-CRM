<?php 
 ...
 if(isset($_SESSION['unique_id'])){
     ...
     $query0 =  mysqli_query($conn, "SELECT * FROM admin WHERE unique_id = '$incoming_id'");
     ...
     $query3 =  mysqli_query($conn, "SELECT * FROM details WHERE unique_id = '$incoming_id'");
     $row3= mysqli_fetch_assoc($query3);



     if ($row0 !== null && $incoming_id == $row0['unique_id']) {
     ...
     }elseif ($row1 !== null && $incoming_id == $row1['unique_id']) {
        ...
        if (mysqli_num_rows($query) > 0) {
            while ($row = mysqli_fetch_assoc($query)) {
                if ($row['outgoing_msg_id'] == $outgoing_id) {
                    ...
                    $smg .= '<div class="chat incoming col-md-6 col-sm-1">
                                <img src="uploaded_img/' . $row['profile'] . '" alt="">
                                <div class="details">
                                    <p>' . $row['msg'] . '</p>
                                </div>
                            </div>';
     ...
     }elseif ($row3 !== null && $incoming_id == $row3['unique_id']) {   

        $outgoing_id = $_SESSION['unique_id'];
        $incoming_id = mysqli_real_escape_string($conn, $_POST['incoming_id']);

        $sql = "SELECT * FROM messages 
        LEFT JOIN details ON details.unique_id = messages.outgoing_msg_id
        WHERE (outgoing_msg_id = {$outgoing_id} AND incoming_msg_id = {$incoming_id})
        OR (outgoing_msg_id = {$incoming_id} AND incoming_msg_id = {$outgoing_id}) ORDER BY msg_id";
       $query = mysqli_query($conn, $sql);
